...
| Capture Trigger Boundary                       | debug.gfxrecon.capture_trigger_boundary                       | STRING  | Select the boundary where GFXReconstruct checks for capture-trigger state changes. Valid values are `frames` and `queue_submits`. Use `queue_submits` for workloads without frame boundaries, such as pure compute. This setting applies only to trigger-based capture; explicit trim ranges use their own boundary. Set before the capture layer initializes. Default is: `frames`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Capture Trigger Boundary Limit                 | debug.gfxrecon.capture_trigger_frames                         | STRING  | Specify a limit on the number of capture boundaries to record after the trigger starts capture. Boundaries are frames by default and queue submits when `debug.gfxrecon.capture_trigger_boundary` is `queue_submits`. Default is: Empty string (no limit) |
...
By default, trigger changes are detected at frame boundaries. Set
`debug.gfxrecon.capture_trigger_boundary` to `queue_submits` to detect them at
queue-submit boundaries instead, for example when capturing a compute workload without frames.
When `queue_submits` is selected, `debug.gfxrecon.capture_trigger_frames` counts queue submits rather than frames.
